...
bronx1<-read_excel("C:/Users/brook/OneDrive/Desktop/Folders/RPI Stuff/Fall 2022/MGMT 6600/Practice/Practice Data/rollingsales_bronx.xls")
bronx1 <- bronx1[-1:-3,]
names(bronx1) <- bronx1[1,]
bronx1 <- bronx1[-1,]
View(bronx1)

#
attach(bronx1) # If you choose to attach, leave out the "data=." in lm regression
SALE.PRICE<-bronx1$`SALE
PRICE`
SALE.PRICE<-as.numeric(SALE.PRICE) 
GROSS.SQUARE.FEET<-as.numeric(bronx1$`GROSS SQUARE FEET`)
LAND.SQUARE.FEET<-as.numeric(bronx1$`LAND SQUARE FEET`)
...
NEIGHBORHOOD <- bronx1$NEIGHBORHOOD
